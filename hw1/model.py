...
class Model(object):
    def __init__(self, input_data, output_data, name, type, batch_size = 64):
        '''
            input: 
            - input_data:       expert_data['observations'], 
        ...
        self.output_placeholder_pre = self.build_model(self.input_placeholder)

        self.loss = tf.reduce_mean(tf.nn.l2_loss(self.output_placeholder_true - self.output_placeholder_pre)) # Scalar
    ...
    def train(self, epochs = 20, train_data = None, test_data = None, number = None):
        '''
            ...
            - output_data:      expert_data['actions'], 
            - name:             args.envname[:-3],
            - type:             "behavior_cloning"/ "DAgger"
        '''
        if train_data is None and test_data is None :
            train_data, test_data = shuffle(self.input_data, self.output_data, random_state = 0)
            print('Train data is nothing, shuffle expert_data')
        else:
            test_data = np.reshape(test_data, (test_data.shape[0], test_data.shape[2]))

        optimizer = tf.train.AdamOptimizer().minimize(self.loss)

        saver = tf.train.Saver() # The Saver class adds ops to save and restore variables to and from checkpoints. It also provides convenience methods to run these ops.

        batch_idxs = len(train_data) // self.batch_size # // operation= division
    ...
    def sample(self, input):
        output = self.sess.run(self.output_placeholder_pre, feed_dict = {self.input_placeholder : np.repeat(input[None, :], 64, axis = 0)})
        return output[0]
